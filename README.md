...
| Library | Full-file | Seek | Bytes | Notes |
| --- | --- | --- | --- | --- |
| [`soundfile`](https://pysoundfile.readthedocs.io/) | yes | yes | yes | libsndfile; the correctness reference |
| [`librosa`](https://librosa.org/) | yes | yes | yes | `offset=`/`duration=` |
| [`scipy.io.wavfile`](https://docs.scipy.org/doc/scipy/reference/generated/scipy.io.wavfile.read.html) | yes | no | yes | WAV only |
| `scipy.io.wavfile` (memmap) | yes | yes | no | WAV only; seek is a memmap slice; memory-mapping requires a real file |
| [`audioread`](https://github.com/beetbox/audioread) | yes | no | no | always yields 16-bit PCM; no in-memory decode API |
| [`pedalboard`](https://github.com/spotify/pedalboard) | yes | yes | yes | `AudioFile.seek` + `.read` |
...
That generates the corpus, runs all three benchmarks, and writes the report and plots to `results/`. The steps are also available separately:
...
Useful flags: `--durations`, `--channels`, `--formats`, `--library` (repeatable), `--bench full|seek|bytes|both|all` (`both` is `full,seek`; `all` is every bench), `--repeat`. `--corpus-dir` (or `PABENCH_CORPUS_DIR`) chooses where the corpus lives, so it can be generated on a specific disk.
